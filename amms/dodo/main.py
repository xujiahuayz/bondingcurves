class Dodo:
    ...
    def _compute_trade_qty_out(
        ...
        diff_one = updated_reserves_in_ix - reserves_regressed_in_ix 
        diff_two = reserves_regressed_in_ix - updated_reserves_in_ix 
        ...
        if updated_reserves_in_ix >= reserves_regressed_in_ix:
            updated_reserves_out_ix = (diff_one+part_1+(
                (diff_one+part_1)**2+4*self.liq_param*part_2*
                                                    (oracle_price*reserves_regressed_out_ix)**2)**(1/2))/(2*oracle_price*part_2)
        else: 
            updated_reserves_out_ix = reserves_regressed_out_ix+(diff_two*(1+self.liq_param*(reserves_regressed_in_ix/updated_reserves_in_ix)
                                                                       -1))/(oracle_price)
